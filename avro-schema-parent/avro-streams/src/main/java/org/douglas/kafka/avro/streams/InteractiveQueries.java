...
public class InteractiveQueries {
    ...
    public List<PipelineMetadata> getMetaData() {
        return streams.streamsMetadataForStore(TopologyProducer.ORDERS_STORE)
                .stream()
    ...
    private ReadOnlyKeyValueStore<String, EnrichedOrder> getWeatherStationStore() {
        while (true) {
            try {
                return streams.store(StoreQueryParameters.fromNameAndType(TopologyProducer.ORDERS_STORE, QueryableStoreTypes.keyValueStore()));
